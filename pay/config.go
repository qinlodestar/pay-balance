...
func init() {
	flag.StringVar(&confFile, "c", "./pay.conf", " set server config file path")
}
...
func InitConfig() (err error) {
	...
	for _, serverID := range gconf.Get("balance.addrs").Keys() {
		addr, err := gconf.Get("balance.addrs").String(serverID)
		if err != nil {
			return err
		}
		Conf.BalanceRPCAddrs[serverID] = addr
	}
	return nil
}
